function Isosurfaces( volume, isovalue, number_index )
{
        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ )
        {
            for ( var x = 0; x < volume.resolution.x - 1; x++ )
            {
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 )
                {
                    ...
                    var v01 = interpolated_vertex( v0, v1, isovalue, number_index );
                    var v23 = interpolated_vertex( v2, v3, isovalue, number_index );
                    var v45 = interpolated_vertex( v4, v5, isovalue, number_index );
    ...
    function interpolated_vertex( v0, v1, s, nindex )
    {
        ...
        if(nindex == 1){
            ...
            var S0 = volume.values[v0.z*volume.resolution.y*volume.resolution.x + v0.y*volume.resolution.x + v0.x][0];
            var S1 = volume.values[v1.z*volume.resolution.y*volume.resolution.x + v1.y*volume.resolution.x + v1.x][0];

            if(S0 < S1){
                var rate = Math.abs(s-S0)/Math.abs(S1-S0);
                var x = rate*(v1.x-v0.x) + v0.x;
                var y = rate*(v1.y-v0.y) + v0.y;
                var z = rate*(v1.z-v0.z) + v0.z;
            }
            else{
                var rate = Math.abs(s-S1)/Math.abs(S0-S1);
                var x = rate*(v0.x-v1.x) + v1.x;
                var y = rate*(v0.y-v1.y) + v1.y;
                var z = rate*(v0.z-v1.z) + v1.z;
            }
